...
def get_pixel_hu(image, intercept, slope):
    """
    Función que recibe la matriz de píxeles DICOM, el interceto y el slope y devuele la matriz en unidades Hounsfield
    """
...
def extract_m_i_s(ruta_imagen):
    """
    Función que extrae la matrix de píxeles el slope y el interceto
    """
...
def load_scan(path):
    ...
    slices = [pydicom.dcmread(path + '/' + s) for s in os.listdir(path)] # se leen los archivos
    slices.sort(key = lambda x: int(x.InstanceNumber)) # se ordenan de acuerdo a InstanceNumber
    try:
...
def extrac_matrix(slices):
    """
    Extrae la matriz de slices que es una lista con archivos dicom leída con pydicom un objeto pydicom.dataset.FileDataset y las devuelve en unidades Hu
    """

    intercept = slices[0].RescaleIntercept
    slope = slices[0].RescaleSlope  
    image = np.stack([s.pixel_array for s in slices]) # junta todos los píxeles array en uno solo 
    image = image.astype(np.int16) # se convierte en una sola matriz  
    image = get_pixel_hu(image, intercept, slope)
    return image

def remov_cama(image, y1 = 350 ,y2= 512):
    """
    Función que permite remover la camilla de un examen de TC
    """
...
def guardar_arrays(carpeta, paciente, array):
    """
    Fución que guarda arrays en formato NPY
    """
    try:
        os.mkdir(carpeta) # crea una carpeta en la misma carpeta donde esta el notebook jupyter
    except:
        print("La carpeta ya existe")
        ruta_carpeta = carpeta + '/'
        np.save(ruta_carpeta + "{}.npy" .format(paciente), array) # almacenamos el array en NPY    
