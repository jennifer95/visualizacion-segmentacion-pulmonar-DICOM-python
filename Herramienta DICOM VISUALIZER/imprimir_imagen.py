...
def get_pixel_hu(image, intercept, slope):
    '''Funcion recibe la  matriz de pixeles DICOM, el interceto y el slope y devuele la matriz en unidades Hounsfield'''
    image = image.copy()
    image[image == -2000] = 0
    if slope != 1:
        image = slope * image
...
def extract_m_i_s(ruta_imagen):
    '''Funcion que extrae la matrix de pixeles el slope y el intercept'''
